...
# Create a DataFrame with a single column 'Keyword'
df_keywords = pd.DataFrame({'Keyword': unique_keywords})

# Save the DataFrame to a CSV file
df_keywords.to_csv('unique_keywords.csv', index=False)
...
# Convert to an array and normalize to get probabilities


co_occurrence_matrix = co_occurrence_matrix / len(texts)

Mp_matrix = pd.DataFrame.sparse.from_spmatrix(co_occurrence_matrix, index=unique_keywords, columns=unique_keywords)
